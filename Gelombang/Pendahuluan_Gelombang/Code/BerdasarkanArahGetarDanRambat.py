...
class BerdasarkanArahGetarDanRambat(MovingCameraScene):
    def construct(self):
        #i know this code look dumb, but atleast this is work :D
        ...
        Line14=Line(start = ORIGIN, end = UP*varup,color=BLUE) #1st
        ...
        Line38=Line(start = ORIGIN, end = UP*varup,color=BLUE) # 2ND 
        ...
        pruh.set_stroke(width = 5).scale(2)
        bruh = Axes(x_range=[0, 2*PI], y_range=[-1.5, 1.5, 1])
        plan = NumberPlane().shift(LEFT)

        #dot = Dot().shift(RIGHT*2.5)

        kotakkanan = RoundedRectangle(corner_radius=0.2,height = 4, width = 6, color = YELLOW).shift(RIGHT*2.5+ UP*0.25)
        kotakkiri = RoundedRectangle(corner_radius=0.2,height = 4, width = 6, color = YELLOW).shift(LEFT*4.5+UP*0.25)

        sine_function = lambda x: (2*np.sin(2*tracker.get_value()-x))
        ax = NumberPlane((-6, 6), (-4, 4),background_line_style={"stroke_opacity": 0}).scale(0.5).shift(LEFT*4.5+UP*0.25)
        sine_plot = always_redraw(lambda : ax.plot(sine_function, color=RED))

        Judul = Text("ARAH GETAR & ARAH RAMBAT",font="Rubik",weight = ULTRABOLD,font_size=40).shift(UP*3+LEFT*0.75)
        Text_GT = Text("Gelombang Transversal",font="Rubik",weight = ULTRABOLD,font_size=30).next_to(kotakkiri,DOWN)
        Text_GL = Text("Gelombang Longitudinal",font="Rubik",weight = ULTRABOLD,font_size=30).next_to(kotakkanan,DOWN)

        #self.add(plan)
        self.add(Judul,pruh,kotakkanan,kotakkiri,ax,Text_GT,Text_GL,sine_plot)

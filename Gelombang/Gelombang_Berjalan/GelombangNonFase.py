...
class GelombangNonFase(MovingCameraScene):
    def construct(self):
        ...
        sefase_text = Tex("Tidak Sefase").next_to(kotak,DOWN)

        def0_text = Tex(r"Dua titik gelombang disebut",r" tidak sefase")
        def1_text = Tex(r"jika jarak kedua titik",r" tidak berupa bilangan bulat,")
        def2_text = Tex(r"atau berupa",r" bilangan ganjil kali setengah panjang gelombangnya")
        def_text = VGroup(def0_text,def1_text,def2_text).arrange(DOWN).shift(DOWN*3.7).scale(0.8)

        beda_fase_nofase = MathTex(r"\Delta \upvarphi = \frac{1}{2},\frac{3}{2},\frac{5}{2}, ...",tex_template=myTemplate).next_to(def_text,DOWN*1.25)
        kotak_nofase = SurroundingRectangle(beda_fase_nofase)
   
        group = VGroup(line,line1,ax,sin,cir,cir1,label,label2).scale(0.6)

        #self.add(NumberPlane())

        self.play(FadeIn(group, shift = UP*0.5),Create(kotak),FadeIn(rumus_gelombang,shift = UP*0.5),Write(sefase_text))
        self.wait(2)
        #self.play(group.animate.shift(UP))
        self.play(self.camera.frame.animate.shift(DOWN*1.9).scale(1.1))
        self.play(Write(def_text))
        self.play(def0_text.animate.set_color_by_tex("tidak sefase", BLUE))
        self.play(def1_text.animate.set_color_by_tex("tidak berupa bilangan bulat", BLUE))
        self.play(def2_text.animate.set_color_by_tex("bilangan ganjil kali setengah panjang gelombangnya", BLUE))
        self.play(Write(beda_fase_nofase),Create(kotak_nofase))
